...
* **The Spec is the Source of Truth:** This document and the numbered specs derived from it define the platform's desired behavior; controller code is generated from them by AI agents (e.g. Claude Code). Code may be edited directly — bug fixes and refactorings are normal — but the governing spec is updated in the same change, so design and runtime artifacts stay consistent.
* **Regeneration is Low Effort:** The specs stay complete enough to generate the implementation from scratch. That makes large changes cheap — switching from Crossplane to plain Kubernetes controllers, say: it is written into the specs, and the affected code is regenerated rather than migrated by hand.
* **Generated Compliance Documentation:** Compliance artifacts such as the test concept or the permission concept are normally written by hand and revised for every release. Here they are generated from the specs by AI instead — cheap to produce, regenerated with each release, and always describing what the platform actually does.
...
To prevent catastrophic data loss via accidental Git operations, the platform enforces a Positive Control model. A resource cannot be destroyed simply by removing its definition file. Instead, deletion is treated as a privileged operation that requires a dedicated "Deletion Request."
