...
> *scope*, not its content. When writing `019-deletion-request.md`, the sole sources of truth
...
> `019-deletion-request.md` has been written, then delete it.
...
- Scope:
  ...
  - The lookup used by 020: find an `Active` request in the same namespace targeting a specific
    resource.
  - The status transition to `Consumed` after a successful deletion, which is written by 020.
...
- **Why 019 sits below 020.** Per design 6.3 Phase 3 the account controller both reads the deletion
