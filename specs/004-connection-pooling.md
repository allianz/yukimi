...
- Constructs the Snowflake connection host from a locator and a cloud-region string, honoring the PrivateLink flag from the base configuration (002).
...
The connection host is built from the account's locator (design.md 3.6) and its cloud-region string (e.g. `aws-eu-central-1`, design.md 3.1). Most regions repeat the cloud as a trailing segment after the region (`aws-eu-west-3` → `eu-west-3.aws`); `eu-central-1` is the one known exception and needs no suffix. A small unexported function handles this with a `switch` for named exceptions and a default case that strips the cloud prefix and reattaches it as a suffix — not a lookup table or Backplane Config (007) entry, since today there is exactly one exception to name. The result is combined with the locator and, depending on the PrivateLink flag (002), either `.privatelink.snowflakecomputing.com` or `.snowflakecomputing.com`.
...
//
// Returns:
//   - User error if region does not match the expected cloud-region format
...
├── pool_test.go
├── host.go          # hostRegionSegment (the region/cloud switch), buildHost
├── host_test.go
...
`internal/snowflake/pool` must never import `internal/snowflake/statement` (005) or `internal/secrets/aws` (003.a). The only imports outside the standard library are `internal/config` (002), `internal/secrets` (003), `internal/errors` (001), and `github.com/snowflakedb/gosnowflake`, pinned at **v1.18.1** — the version `specs/notes-snowflake-sql-mechanics.md`'s driver findings were verified against; an upgrade means re-verifying those findings before relying on them.

## Error Classification

**User Errors** (use `errors.NewUserError()`):
- Malformed cloud-region string passed to `TenantAccount`: `region 'Frankfurt!' does not match the expected cloud-region format (expected: aws-eu-central-1)`

This package re-validates the region shape independently of whatever validation the caller (a guardrail, 008, or 002's own shape check on `OrgAdminAccountRegion`) already performed — the same reasoning 003 gives for re-validating every secret path segment independently of the caller.
...
- **SC-006**: `hostRegionSegment("aws-eu-central-1")` returns `"eu-central-1"`; `hostRegionSegment("aws-eu-west-3")` returns `"eu-west-3.aws"`.
- **SC-007**: `buildHost` appends `.privatelink.snowflakecomputing.com` when `UsePrivateLink` is true and `.snowflakecomputing.com` when false, and the locator forms the leading label in both.
- **SC-008**: `TenantAccount` returns a user error for a `region` missing its cloud prefix (e.g. `"eu-central-1"`) or otherwise malformed, and never attempts a connection in that case.
...
- **SC-017**: `internal/snowflake/pool` imports `internal/config`, `internal/secrets`, `internal/errors`, and `github.com/snowflakedb/gosnowflake` among dependencies with an `internal/` boundary or a new `go.mod` entry — never `internal/secrets/aws` and never `internal/snowflake/statement`, grep-provable.
...
- **Product design**: `specs/design.md`, §3.6 (`CREATE ACCOUNT`, the locator, PrivateLink), §3.11 (organization vs. account-level privilege step-down), §3.11.1 (the tenant secret path this package's cache key mirrors), §3.12 (CRD name vs. resolved Snowflake name), §6.3 (`DROP ACCOUNT`), Appendix B X1 (the `platform` service user).
