...
> *scope*, not its content. When writing `015-auth-module.md`, the sole sources of truth are
> `specs/design.md` and the prompt given at spec-writing time — rework, restructure, or
> discard anything below freely. Please keep this file up to date until `015-auth-module.md`
...
- Scope:
  ...
  - Validation: an entry naming **neither** method is a validation error. `reason` is recorded in the
    ...
    here — they are governed by their own network policies (014).
  - Rejection behavior mirrors 014: a rejected entry leaves the user on the SSO-only baseline and is
    reported on `Synced`.
...
- **Same overwrite-and-prune contract as 014.** Re-assert auth policies and bindings in full on every
...
- **Name the enumeration handle.** Pruning works by listing the policies this module owns and taking a
  set difference, so 015 needs its own naming convention for per-user auth policies (014 has `CUSTOM_`;
  015 has nothing sketched yet). Deciding that name is part of writing 015.
...
- **A rejected exception never stops the run.** Only 012 ever aborts the run; 015 never calls
...
- **015 has no guardrail dependency.** Design.md's guardrails section (3.3) does not constrain
