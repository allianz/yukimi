...
//
// Message Guidelines:
//   - Include field path: spec.region, spec.customNetworkRules.serviceUsers.tu_airflow[0].allowedIPs[0]
...
- Malformed CIDR: `Invalid CIDR '10.0.0.256/24' in spec.customNetworkRules.serviceUsers.tu_airflow[0].allowedIPs[0]: not a valid IP range`
...
// Validate CIDR blocks. Each service user under customNetworkRules.serviceUsers holds a list
    ...
    for i, entry := range entries {
        for j, cidr := range entry.AllowedIPs {
            if _, _, err := net.ParseCIDR(cidr); err != nil {
                return errors.NewUserError(fmt.Sprintf(
                    "Invalid CIDR '%s' in spec.customNetworkRules.serviceUsers.%s[%d].allowedIPs[%d]: not a valid IP range",
                    cidr, user, i, j))
            }
        }
    }
