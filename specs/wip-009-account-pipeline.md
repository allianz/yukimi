...
### D-003 — An explicit structural slot, enforced by the constructor signature
...
**Decision**: The constructor takes the structural module separately from the rest:

```go
func New(structural Module, rest ...Module) *Pipeline
```

`Observation.Exists` is the structural module's answer alone; no other module contributes to it. The
pipeline never learns *which* module this is beyond its position in the signature, so
`internal/account` still imports no implementor (the scope note's hard rule holds).

**Rationale**: The alternatives were a `Structural() bool` method on `Module` (silent to the reader,
and admits zero or two structural modules) or a per-outcome abort flag any module could set (turns one
architectural fact into a per-call decision each module spec would have to restate). Splitting the
constructor argument makes "exactly one structural module, and it is first" unrepresentable-otherwise,
and it is self-documenting at the single call site in 018.
...
### D-006 — Any non-`Done` outcome from the structural module stops the run

**Question**: Does only `Failed` from 010 abort, or does any non-`Done` outcome?

**Decision**: Any non-`Done` outcome — `Pending` and `Rejected` included. All three mean there is no
usable account: `Failed` means `CREATE ACCOUNT` errored, `Rejected` means the request was refused, and
`Pending` means the account is not yet ready to accept a `platform`-user connection. In every case
each later module would fail on its first statement for a reason that has nothing to do with that
module.
...
subsystem. The distinction the design cares about is structural-versus-not (D-003), not which of the
three non-`Done` states occurred.
...
### D-008 — A non-structural module's failure never stops the run

**Question**: Does a `Rejected` or `Failed` outcome from 011–016 stop the modules after it?

**Decision**: No. Every non-structural module runs on every `Apply`, regardless of what the ones
before it returned. Only the structural module can abort (D-006).
...
configuration from being applied. Generalizing that from network rules to all non-structural modules
avoids a per-module abort policy nobody can keep straight.
...
module after the structural one get a connection on the very first reconcile?

**Decision**: The context owns the locator and hands out connections lazily:

```go
func (c *Context) Locator() string
func (c *Context) SetLocator(locator string)              // structural module only
...
018 seeds the locator from `status.accountLocator` when it is already set. The structural module calls
...
consequence: because 015 runs after the structural module, nothing is emitted while `CREATE ACCOUNT`
...
could run on — the structural module needs org-admin, everything else runs as the account's own
...
**Why it is hard**: D-018 puts emission in 015, which runs after the structural module, and D-006
aborts the run when the structural module is not `Done`. So a persistently failing `CREATE ACCOUNT`
also delays the identity sync — and §4.3's sync is measured in tens of minutes, so the delay compounds
onto an already slow path. Design §4.3 says the request is "emitted early, never blocking", which
this arguably weakens.

**Options and trade-offs**:
- *Accept it* (current): consistent with D-006, no special case. Costs sync latency exactly when the
  account is already in trouble.
- *Emit before the structural module*: honours "emitted early" literally. Requires either a module
  ordered before the structural one (which the abort rule would then have to exempt) or 018 emitting
  directly — both were rejected in D-018.
...
- **010 (account module)** — is the structural module (D-003). Its `Observe` is mandatory and is the
  sole source of `Observation.Exists`. It must call `mc.SetLocator()` immediately after
  `CREATE ACCOUNT` returns, or every later module loses its connection (D-013). Any non-`Done` outcome
  it returns stops the run (D-006), so it must not use `Pending` for states that are in fact fine.
